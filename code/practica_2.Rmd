...
És un conjunt de dades obtingut de la web Kaggle i construit per 29 atributs, dels quals hi ha un d’ells que és l’identificador únic del registre, 25 atributs numèrics enters 
...
Procedim a realitzar un anàlisis de correlació amb les diferents variables i així verificar si hi ha alguna variablme amb que les varibles 
...
tinguin una dependència prou gran per creure que pot influir amb el seu valor.
...
Les variables amb més correlació amb la quantitat de fruita comprada són els que estàn més a prop dels valors -1 i 1. 
...
Les variables amb més correlació amb la quantitat de carn comprada són els que estàn més a prop dels valors -1 i 1. 
...
Les variables amb més correlació amb la quantitat de peix comprada són els que estàn més a prop dels valors -1 i 1. 
...
Les variales amb més correlació amb la quantitat de dolços comprada són els que estàn més a prop dels valors -1 i 1. 
...
Com que quasi totes les relacions entre les variables del dataset original no són homogènees, utilitzarem el tests de kruskal entre els diferentes variables i les variables d'estudi, revisant quin valor de p-value dona.

Si és menor a 0,05 concluirem que els valors de la variable d'estudi és variant segons els diferents valors de l'altra variable.
...
Enaquest cas numCatalogPurchases té un valor superior a 0,05. Un resultat coheremt amb el resultat que ens ha donat anteriorment revisant la correlació de les variables.
...
Per molt que no siguin normals, en els casos que tenim un tamany prou gran, podem utilitzar la prova. 
...
Revisem el número d'observaciones per dataset:
...
En el nostre cas, totes les mostres són majors de 30 registres per tant podem utilitzar l'anàlisis de contrast de t-student amb aquests conjunts de dades:

Fem un estudi reswpecte el que es gasta en fruita els diferents subconjunts que anteriorment hem calculat:
...
```
    En aquest cas veiem que els p-valors resultants de la prova t de Student més alts són els resultants dels clients que tenen estudis de graduació i de cicle per a qualsevol estat marital. Això vol dir que no s'observen diferències estadísticament significatives.
...
En aques cas, i segons els estudis anteriors, ens centrarem amb les segÜents variables: 

Farem un estudi de regresió lineal entre la variable MntMeatProducts i
...
En aquest cas veiem que tots tenen una quoficient de determinació baix, per tant ens quedem l'últim model per fer la predicció del número de diners gastats per un client amb carn.
